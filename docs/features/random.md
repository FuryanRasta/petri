...
This specification describes the usage and scope of random numbers on Petri. This feature is currently in beta and please assess the risk yourself before using.

## Concepts

Currently, Petri provides two random number generation methods: PRNG and TRNG.
...
On the basis of PRNG, the external random number factor `Oracle Seed` is introduced using the oracle machine method, implemented through Petri Service.
...
- [Request Random Number](../cli-client/rand.md#petri-tx-random-request-random)
- [Query Random Number](../cli-client/rand.md#petri-query-random-random)
- [Query Random Queue](../cli-client/rand.md#petri-query-random-queue)
