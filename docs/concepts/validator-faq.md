...
- `unbonding`:
  ...
  - Validator is out of the top 100 bonded and become a Candidate, the validator can delegate more PETRI to himself to be in top 100, then he will get bonded automatically.
...
- It is generated when the node is created by `petri init`
...
- Query the associated hex address
  ...
  ```bash
  petri status | jq .validator_info.address
  ```
...
This key is created from `petri` and used to sign transactions. Application keys are associated with a public key prefixed by `iap` and an address prefixed by `iaa`. Both are derived from account keys generated by `petri keys add`.
...
If you are using the software sign (which is the default signing method of tendermint), your [Tendermint Key](#tendermint-key) is located in `<petri-home>/config/priv_validator.json`. The easiest way is to backup the whole config folder.
...
### Is there a minimum amount of PETRI that must be delegated to be an active (=bonded) validator

The minimum amount is `1 petri`.
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of bonded PETRI

The validator that is selected to propose the next block is called proposer. Each proposer is selected deterministically, and the frequency of being chosen is proportional to the voting power (i.e. amount of bonded PETRI) of the validator. For example, if the total bonded stake across all validators is 100 PETRI and a validator's total stake is 10, then this validator will propose ~10% of the blocks.
...
- Validator operator address, which is an [Application Key](#application-key) that you used to create the validator
  ...
  ```bash
  petri keys show MyKey --bech=val
  ```
...
4. [Edit your validator](../cli-client/staking.md#petri-tx-staking-edit-validator) and specify `--identity=<the_16_digit_string>`
...
First, if `petri` is not running, start it up again:

```bash
petri start
```

Wait for your full node to catch up to the latest block. Then, you can [unjail your validator](../cli-client/slashing.md#petri-tx-slashing-unjail) if you have been jailed over the [DowntimeJailDuration](gov-params.md#parameters-in-slashing)
...
### My `petri` crashes because of `too many open files`

The default number of files Linux can open (per-process) is `1024`. `petri` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed, only effective for the current session) and then restart the process with `petri start`. If you are using `systemd` or another process manager to launch `petri` this may require some configuration at that level.

- A sample `systemd` file to fix this issue is below:

    ```toml
    # /etc/systemd/system/petri.service
    [Unit]
    Description=PETRI Hub Node
    ...
    ExecStart=/home/ubuntu/go/bin/petri start
    Restart=on-failure
    RestartSec=3
    LimitNOFILE=65535
...
- Stop the node
- Replace the current `<petri-home>/config/priv_validator.json` with the one you backed up
- Confirm the `Consensus Pubkey` is correct via `petri tendermint show-validator --home=<petri-home>`
- Start the node
- Check `voting_power` should now be greater than 0: `petri status`

#### What if I lost my Tendermint Key

That means you have **Lost Your Validator Forever!** You can only create a new one and [redelegate](../cli-client/staking.md#petri-tx-staking-redelegate) all of your shares to the new Validator.
